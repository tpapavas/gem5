...
TraceLoaderGem5::axievent(int* waiting_for_gem5_mem) {
    ...
    // todo: because nvdla compiler has addr reuse, 0x9: all data
    // in nvdla.cpp, offset can only be 0x5 or 0x8.
    // but here use 0x8 to denote read-only variables, 0x9: write-only
    // and 0xa to denote read-and-write variables
    // so that different caching policies can be adopted
    AXIResponder *axi;
    ...
    case AXI_DUMPMEM: {
        ...
        if (!*waiting_for_gem5_mem) {
            ...
            *waiting_for_gem5_mem = 1;
            ...
            uint32_t got_response = axi->read_response_for_traceLoaderGem5(
                                      txn_start_addr, read_response_buffer);

            if (got_response) {
                uint32_t old_op_addr = op.addr;
                // first we update the op info,
                // which is related to the next txn (if there is)
                if (!aligned) {
                ...
                write(fd,
                    read_response_buffer + (old_op_addr - txn_start_addr),
                    bytes_to_write);
                close(fd);

                if (op.len <= 0) {
                    if (axi->getRequestsOnFlight() != 0) {
                        printf("op.len == 0 is not synchronous "
                            "with inflight_req_order.empty().\n");
                        abort();
                    }
                    *waiting_for_gem5_mem = 0;
                    ...
                    while (1) {
                        ...
                        if (check_byte != *buf && matched) {
                            printf("Memory dump does not match golden answer "
                                   "at byte %d:exp 0x%02x, got 0x%02x, "
