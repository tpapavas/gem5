...
class TraceLoaderGem5
{
    ...
    uint32_t base_addr;
    // this value will be valid after trace->load(),
    // where we find 0xff as the end of trace
    uint32_t trace_size;

    int _test_passed;

public:
    // this value will be valid right after receiving CPU launch accel pkt
    uint32_t trace_and_rd_log_size;
    enum stop_type
    {
        TRACE_CONTINUE = 0,
        TRACE_AXIEVENT,
