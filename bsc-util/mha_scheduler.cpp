...
// input head_id starts from 0, but in file names start from '1'
size_t get_trace_from_file(const std::string& dir, int head_id, char* dst) {
    // open trace file
    char subscript_char = '1' + head_id;
    std::string trace_file_name = dir + std::string("trace.bin_")
        + subscript_char;
    ...
    std::string var_log_file_name = dir + std::string("rd_only_var_log_")
        + subscript_char;
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < worker_num; i++) {
        m5_start_accel_id((uint64_t)region_nvdlas[i],
            trace_sizes[i], (uint64_t)region_nvdlas[i], i);
        need_waiting[i] = true;
    }

    while (need_waiting[0] || need_waiting[1]
      || need_waiting[2] || need_waiting[3]) {
        for (int i = 0; i < worker_num; i++) {
