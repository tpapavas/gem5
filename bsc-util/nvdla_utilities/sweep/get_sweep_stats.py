...
class PerformanceCollector:
    def __init__(
        self, get_root_dir, jsons_dir="", pr_stats=None, sh_stats=None
    ):
        ...
        self.stats_pr_header = (
            ["nvdla_cycles", "memory_cycles"] if pr_stats is None else pr_stats
        )
        self.stats_sh_header = (
            [
                "num_dma_prefetch",
                "num_dma",
                "host_seconds",
        ...
        self.stat_header2identifier = (
            {  # including both private and shared stats' headers
        ...
        for root, dirs, files in sorted(
            os.walk(get_root_dir), key=natural_keys
        ):
            if "run.sh" in files:
        ...
        # get self.var_header
        jsons_dir = (
            os.path.join(get_root_dir, "../jsons")
            if jsons_dir == ""
            else jsons_dir
        )
        assert os.path.exists(
            jsons_dir
        ), f"path '{jsons_dir}' does not exist!\n"
            ...
            for file in files:
                if ".json" in file:
                    ...
                    for var, to_sweep_values in data.items():
                        assert (
                            type(to_sweep_values) == list
                        ), f"weird type in json file.\n"
    ...
    def init_testcase(self, sweep_dir):
        ...
        self.dbb_inflight = [
            [(0, 0)] for _ in range(self.max_num_nvdla)
        ]  # [[(cycle_id, num), ...], ...]
        ...
        while byte_id < len(self.axilog):
            int64_0 = int.from_bytes(
                self.axilog[byte_id : byte_id + 8], byteorder="little"
            )
            byte_id += 8
            int64_1 = int.from_bytes(
                self.axilog[byte_id : byte_id + 8], byteorder="little"
            )
            byte_id += 8
            ...
            if print_cmd == 0x8:  # dma_prefetch
                ...
                if name == ord("D"):
                    to_append = (
                        time,
                        self.dbb_inflight[nvdla][-1][-1] + burst + 1,
                    )
                    ...
                    to_append = (
                        time,
                        self.cvsram_inflight[nvdla][-1][-1] + burst + 1,
                    )
    ...
    def get_host_seconds(self):
        lines = os.popen(
            "cd "
            + self.sweep_dir
            + ' && cat stdout | grep -E "simulation time: [0-9]+ seconds"'
        ).readlines()
        time = 0
        for line in lines:
    ...
    def get_simulated_seconds(self):
        ...
        st_num = (
            os.popen(
                "cd "
                + self.sweep_dir
                + ' && cat system.terminal | grep "start_time =" -c'
            )
            .readlines()[0]
            .strip("\n")
        )
        ed_num = (
            os.popen(
                "cd "
                + self.sweep_dir
                + ' && cat system.terminal | grep "end_time =" -c'
            )
            .readlines()[0]
            .strip("\n")
        )
        pipeline_sign = (
            os.popen(
                "cd "
                + self.sweep_dir
                + ' && cat system.terminal | grep "finished all ops" -c'
            )
            .readlines()[0]
            .strip("\n")
        )
        ...
        if int(st_num) != 0 and int(ed_num) != 0:  # single core pattern
            st = (
                os.popen(
                    "cd "
                    + self.sweep_dir
                    + ' && cat system.terminal | grep "start_time ="'
                )
                .readlines()[0]
                .strip("\n")
            )
            ...
            ed = (
                os.popen(
                    "cd "
                    + self.sweep_dir
                    + ' && cat system.terminal | grep "end_time ="'
                )
                .readlines()[0]
                .strip("\n")
            )
            ...
            time_records = os.popen(
                "cd "
                + self.sweep_dir
                + ' && cat system.terminal | grep " at t = "'
            ).readlines()
            st_time_match = re.search(
                r"NVDLA [0-9]+ launched batch [0-9]+ at t = ([0-9\.]+)",
                time_records[0],
            )
            ed_time_match = re.search(
                r"NVDLA [0-9]+ finished batch [0-9]+ at t = ([0-9\.]+)",
                time_records[-1],
            )
            if st_time_match is None or ed_time_match is None:
                return "0"
            return str(
                float(ed_time_match.group(1)) - float(st_time_match.group(1))
            )
        else:
            return "0"
    ...
    def get_nvdla_cycles(self):
        ...
        stats_txt_path = os.path.abspath(
            os.path.join(self.sweep_dir, "stats.txt")
        )
        if exists(stats_txt_path):
            stat_lines = os.popen(
                "cat " + stats_txt_path + " | grep nvdla_cycles"
            ).readlines()
        else:
            stat_lines = []
        ...
        for line in stat_lines:
            if "nvdla_cycles" in line:
                ...
                stat_val_match = re.search(
                    r"%s\s+([0-9]+)\s+" % "nvdla_cycles", line
                )
    ...
    def get_memory_cycles(self):
        ...
        for nvdla in range(self.max_num_nvdla):
            begin_time = min(
                self.dbb_inflight[nvdla][0][0],
                self.cvsram_inflight[nvdla][0][0],
            )
            end_time = max(
                self.dbb_inflight[nvdla][-1][0],
                self.cvsram_inflight[nvdla][-1][0],
            )
            ...
            for i in range(len(self.dbb_inflight[nvdla]) - 1):
                if self.dbb_inflight[nvdla][i][1] == 0 and (
                    self.dbb_inflight[nvdla][i + 1][1] != 0
                    or self.dbb_inflight[nvdla][i + 1][0]
                    - self.dbb_inflight[nvdla][i][0]
                    > 10
                ):
                    ...
                    zero_dbb_intervals.append(
                        (
                            self.dbb_inflight[nvdla][i][0],
                            self.dbb_inflight[nvdla][i + 1][0],
                        )
                    )

            for i in range(len(self.cvsram_inflight[nvdla]) - 1):
                if self.cvsram_inflight[nvdla][i][1] == 0 and (
                    self.cvsram_inflight[nvdla][i + 1][1] != 0
                    or self.cvsram_inflight[nvdla][i + 1][0]
                    - self.cvsram_inflight[nvdla][i][0]
                    > 10
                ):
                    zero_cvsram_intervals.append(
                        (
                            self.cvsram_inflight[nvdla][i][0],
                            self.cvsram_inflight[nvdla][i + 1][0],
                        )
                    )
            ...
            for interval in zero_dbb_intervals:
                while last_cvsram_int_id < len(zero_cvsram_intervals):
                    ...
                    if (
                        cvsram_int[1] <= interval[0]
                    ):  # cvsram_interval is to the left of dbb_interval
                        last_cvsram_int_id += 1
                        continue
                    elif (
                        cvsram_int[0] >= interval[1]
                    ):  # cvsram_interval is to the right of dbb_interval
                        ...
                        while try_cvsram_id < len(zero_cvsram_intervals):
                            try_cvsram_int = zero_cvsram_intervals[
                                try_cvsram_id
                            ]
                            if (
                                try_cvsram_int[0] >= interval[1]
                            ):  # try_cvsram_interval is to the right of dbb_interval
                                break
    ...
    def write_to_sweep_dir(self):
        ...
        sweep_name = os.path.relpath(
            os.path.abspath(os.path.join(self.sweep_dir, "..")),
            self.get_root_dir,
        )
        this_pt_list = (
            [sweep_name, sweep_id] + self.get_var_val() + self.get_stat_val()
        )
        this_pt_list = [str(e) for e in this_pt_list]
        this_pt_table = [
            self.sweep_header + self.var_header + self.stats_header,
            this_pt_list,
        ]
        with open(
            os.path.join(self.sweep_dir, sweep_id + "_summary.csv"), "w"
        ) as fp:
...
def summarize(options):
    ...
    for root, dirs, files in sorted(
        os.walk(options.get_root_dir), key=natural_keys
    ):
        if "run.sh" in files:
            ...
            sweep_id = dir_level[-1]
            if (
                sweep_id + "_summary.csv" in files
            ):  # statistics have been generated right after simulation
    ...
    with open(
        os.path.join(options.out_dir, options.out_prefix + "_summary.csv"), "w"
    ) as fp:
