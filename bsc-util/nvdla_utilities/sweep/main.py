...
def main():
    ...
    parser.add_argument(
        "--jsons-dir",
        help="Path to the directory including sweep parameter JSONs.",
        required=True,
    )
    parser.add_argument(
        "--out-dir",
        help="Output directory for storing simulation logs.",
        required=True,
    )
    parser.add_argument(
        "--vp-out-dir",
        help="should be the same with '--out-dir' for caffe2trace.py "
        "and pipeline_compile.py. This directory holds input.txn, sc.log mem trace, etc.",
        required=True,
    )
    parser.add_argument(
        "--sim-dir",
        help="The directory inside gem5 disk image for the simulated system to read data from. "
        "e.g., /home/lenet/",
        required=True,
    )
    ...
    parser.add_argument(
        "--home",
        default="~",
        help="Absolute path to the home directory. "
    ...
    parser.add_argument(
        "--disk-image",
        default="/home/ngiannopoulos/Phd/NVDLA/gem5_linux_images/ubuntu-18.04-arm64-docker_MY.img",
    ...
    parser.add_argument(
        "--scheduler",
        help="The name of binary to run on simulated system,"
        "scheduler=xx means path_to_scheduler_in_disk_image=/home/xx",
        required=True,
    )
    parser.add_argument(
        "--pipeline-batches",
    ...
    if not args.gem5_binary:
        fast_path = os.path.abspath(
            os.path.join(
                os.path.dirname(__file__), "../../../build/ARM/gem5.fast"
            )
        )
        if os.path.exists(fast_path):
            args.gem5_binary = fast_path
        else:
            opt_path = os.path.abspath(
                os.path.join(
                    os.path.dirname(__file__), "../../../build/ARM/gem5.opt"
                )
            )
            assert os.path.exists(
                opt_path
            ), f"gem5.fast/.opt binary does not exist!"
