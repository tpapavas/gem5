    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--little-cpu-clock\s+([0-9a-zA-Z\_]+)", line
            ).group(1)
...
class DMAEnableParam(BaseParam):
    ...
    def is_meaningful(self, type_val_pairs):
        if (
            type_val_pairs[UseFakeMemParam] != ""
            or type_val_pairs[AddAccelPrivateCacheParam] != ""
            or type_val_pairs[AddAccelSharedCacheParam] != ""
...
class BufferModeParam(BaseParam):
    ...
    def is_meaningful(self, type_val_pairs):
        if (
            type_val_pairs[DMAEnableParam] != "--dma-enable"
            and type_val_pairs[AddAccelPrivateCacheParam]
            != "--add-accel-private-cache"
            and type_val_pairs[AddAccelSharedCacheParam]
        ...
        if type_val_pairs[PftEnableParam] != "--pft-enable" and (
            self.curr_sweep_value() == "pft"
            or self.curr_sweep_value() == "pft-cut"
        ):
...
class EmbedSPMSizeParam(BaseParam):
    def __init__(self, name, sweep_vals):
        new_sweep_vals = [
            re.sub(r"([0-9])KB", r"\1kB", val) for val in sweep_vals
        ]
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(r"--embed-spm-size\s+([0-9a-zA-Z]+)", line).group(
                1
            )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--embed-spm-assoc\s+([0-9a-zA-Z]+)", line
            ).group(1)
...
class AddAccelPrivateCacheParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"add-accel-private-cache": self.curr_sweep_value()},
        )

    def is_meaningful(self, type_val_pairs):
        if (
            type_val_pairs[DMAEnableParam] != ""
            or type_val_pairs[UseFakeMemParam] != ""
        ) and self.curr_sweep_value() != "":
            return False
        return True
...
class AccelPrCacheSizeParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-pr-cache-size": self.curr_sweep_value()},
        )

    def is_meaningful(self, type_val_pairs):
        if (
            type_val_pairs[AddAccelPrivateCacheParam]
            != "--add-accel-private-cache"
            and self.curr_sweep_value() != self._sweep_vals[0]
        ):
            return False
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-pr-cache-size\s+([0-9a-zA-Z]+)", line
            ).group(1)
...
class AccelPrCacheAssocParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-pr-cache-assoc": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(r"--accel-pr-cache-assoc\s+([0-9]+)", line).group(
                1
            )
...
class AccelPrCacheTagLatParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-pr-cache-tag-lat": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-pr-cache-tag-lat\s+([0-9]+)", line
            ).group(1)
...
class AccelPrCacheDatLatParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-pr-cache-dat-lat": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-pr-cache-dat-lat\s+([0-9]+)", line
            ).group(1)
...
class AccelPrCacheRespLatParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-pr-cache-resp-lat": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-pr-cache-resp-lat\s+([0-9]+)", line
            ).group(1)
...
class AccelPrCacheMshrParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-pr-cache-mshr": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(r"--accel-pr-cache-mshr\s+([0-9]+)", line).group(
                1
            )
...
class AccelPrCacheTgtsPerMshrParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-pr-cache-tgts-per-mshr": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-pr-cache-tgts-per-mshr\s+([0-9]+)", line
            ).group(1)
...
class AccelPrCacheWrBufParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-pr-cache-wr-buf": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-pr-cache-wr-buf\s+([0-9]+)", line
            ).group(1)
...
class AccelPrCacheClusParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-pr-cache-clus": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-pr-cache-clus\s+([a-zA-Z\_]+)", line
            ).group(1)
...
class AddAccelSharedCacheParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"add-accel-shared-cache": self.curr_sweep_value()},
        )
...
class AccelShCacheSizeParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-sh-cache-size": self.curr_sweep_value()},
        )

    def is_meaningful(self, type_val_pairs):
        if (
            type_val_pairs[AddAccelSharedCacheParam]
            != "--add-accel-shared-cache"
            and self.curr_sweep_value() != self._sweep_vals[0]
        ):
            return False
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-sh-cache-size\s+([0-9a-zA-Z]+)", line
            ).group(1)
...
class AccelShCacheAssocParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-sh-cache-assoc": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(r"--accel-sh-cache-assoc\s+([0-9]+)", line).group(
                1
            )
...
class AccelShCacheTagLatParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-sh-cache-tag-lat": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-sh-cache-tag-lat\s+([0-9]+)", line
            ).group(1)
...
class AccelShCacheDatLatParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-sh-cache-dat-lat": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-sh-cache-dat-lat\s+([0-9]+)", line
            ).group(1)
...
class AccelShCacheRespLatParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-sh-cache-resp-lat": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-sh-cache-resp-lat\s+([0-9]+)", line
            ).group(1)
...
class AccelShCacheMshrParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-sh-cache-mshr": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(r"--accel-sh-cache-mshr\s+([0-9]+)", line).group(
                1
            )
...
class AccelShCacheTgtsPerMshrParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-sh-cache-tgts-per-mshr": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-sh-cache-tgts-per-mshr\s+([0-9]+)", line
            ).group(1)
...
class AccelShCacheWrBufParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-sh-cache-wr-buf": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-sh-cache-wr-buf\s+([0-9]+)", line
            ).group(1)
...
class AccelShCacheClusParam(BaseParam):
    ...
    def apply(self, point_dir):
        change_config_file(
            point_dir,
            "run.sh",
            {"accel-sh-cache-clus": self.curr_sweep_value()},
        )
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--accel-sh-cache-clus\s+([a-zA-Z\_]+)", line
            ).group(1)
...
class PftEnableParam(BaseParam):
    ...
    def is_meaningful(self, type_val_pairs):
        if (
            type_val_pairs[UseFakeMemParam] == "--use-fake-mem"
            and self.curr_sweep_value() != self._sweep_vals[0]
        ):
            return False
        return True
...
class PftThresholdParam(BaseParam):
    ...
    def is_meaningful(self, type_val_pairs):
        if (
            type_val_pairs[PftEnableParam] != "--pft-enable"
            and self.curr_sweep_value() != self._sweep_vals[0]
        ):
            return False
        return True
...
class CVSRAMSizeParam(BaseParam):
    ...
    def is_meaningful(self, type_val_pairs):
        if (
            type_val_pairs[CVSRAMEnableParam] != "--cvsram-enable"
            and self.curr_sweep_value() != self._sweep_vals[0]
        ):
            return False
        if (
            type_val_pairs[RemapperParam] == "Identity"
            and self.curr_sweep_value() != self._sweep_vals[0]
        ):
            return False
        return True
    ...
    def get(cls, point_dir, run_sh_lines):
        for line in run_sh_lines:
            ...
            if pos == -1:
                continue
            return re.search(
                r"--cvsram-bandwidth\s+([0-9a-zA-Z\/]+)", line
            ).group(1)
...
class RemapperParam(BaseParam):
    ...
    def is_meaningful(self, type_val_pairs):
        if type_val_pairs[NumNVDLAParam] == 1 and eval(
            "issubclass("
            + self.curr_sweep_value()
        ...
        if type_val_pairs[NumNVDLAParam] != 1 and eval(
            ...
            return False
        if (
            type_val_pairs[NumNVDLAParam] != 1
            and self.curr_sweep_value() == "Identity"
        ):
            return False
        if type_val_pairs[CVSRAMEnableParam] != "--cvsram-enable" and eval(
            "issubclass("
            + self.curr_sweep_value()
