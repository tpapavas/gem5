...
NVDLA_VP_BIN_PATH = "/home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/"
...
def parse_args():
    ...
    parser.add_argument(
        "--nvdla-compiler",
        default="/home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/nvdla_compiler",
        help="Path to NVDLA compiler",
    )
    parser.add_argument(
        "--qemu-bin",
        default="/home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/aarch64_toplevel",
    ...
    parser.add_argument(
        "--qemu-lua",
        default="/home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/conf/aarch64_nvdla.lua",
        help="Path to configuration file of running qemu. Using the one in docker image is ok.",
    )
    parser.add_argument(
        "--out-dir",
        default="/home/tpapavasileiou/tools/GEM5-NVDLA/nvdla/traces/tp-lenet-caffe/",
    ...
    parser.add_argument(
        ...
        default="/home/tpapavasileiou/tools/MNIST-Dataset/t10k-imgs/5/image5223.pgm",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to get true data from sc.log",
    )
    parser.add_argument(
        "--dump-results",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to assume the presence of sc.log "
...
def get_loadable(options):
    ...
    assert os.path.exists(options.nvdla_compiler)
    compile_log_path = os.path.abspath(
        os.path.join(options.out_dir, "compile_log")
    )

    curr_lib_path = ""
    if "LD_LIBRARY_PATH" in os.environ.keys():
        curr_lib_path = os.environ["LD_LIBRARY_PATH"]
    os.environ[
        "LD_LIBRARY_PATH"
    ] = f"{curr_lib_path}:{NVDLA_VP_BIN_PATH}/usr/lib/x86_64-linux-gnu:{NVDLA_VP_BIN_PATH}/usr/lib:{NVDLA_VP_BIN_PATH}/usr/local/systemc-2.3.0/lib-linux64/:{NVDLA_VP_BIN_PATH}/usr/local/nvdla/compiler"
    ...
    cmd = (
        ...
        + " -o "
        + options.model_name
        + " --cprecision fp16 --configtarget nv_full --informat nchw --prototxt "
        + os.path.abspath(options.prototxt)
        + " --caffemodel "
        + os.path.abspath(options.caffemodel)
        + " > "
        + compile_log_path
        + " 2>&1 && mv fast-math.nvdla "
        + options.model_name
        + ".nvdla && mv "
        + options.model_name
        + ".nvdla "
...
def run_qemu(options):
    ...
    if options.true_data:  # prepare real input data if necessary
        ...
        image_str = " --image usr/local/nvdla/" + file_name + " "
        os.system(
            "cp "
            + os.path.abspath(options.image)
            + " /home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/usr/local/nvdla/"
        )
    ...
    qemu_run_template = """#!%(path_to_vp_bin)s/usr/bin/expect

# starts guest vm, run benchmarks, poweroff
set timeout -1

# Assign a variable to the log file
log_file %(path_to_vp_bin)s/qemu_log
...
""" % {
        "path_to_vp_bin": NVDLA_VP_BIN_PATH,
        "bin": options.qemu_bin,
        "lua": options.qemu_lua,
        "loadable": options.model_name + ".nvdla",
        "image": image_str,
    }
    qemu_run_template.format(options.qemu_bin, options.qemu_lua)
    with open(
        "/home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/images/linux-4.13.3/qemu_run.exp",
        "w",
    ) as f:
        f.write(qemu_run_template)
    cmd = "cd /home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/ && ./images/linux-4.13.3/qemu_run.exp"
    if os.path.exists("/home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/qemu_log"):
        os.system(
            "mv /home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/qemu_log /home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/qemu_log_bkp"
        )
...
def parse_mixed_type_trace(rd_wr_trace_file):
    ...
    fig.savefig(
        os.path.join(
            os.path.dirname(rd_wr_trace_file), rd_wr_trace_file + ".png"
        ),
        dpi=240,
    )
...
def process_log(options):
    if not options.convert_only:
        os.system(
            "cd /home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/ && mv sc.log "
            + options.out_dir
        )
        os.system(
            "cd /home/tpapavasileiou/tools/GEM5-NVDLA/vp-bin/ && mv qemu_log "
            + options.out_dir
        )
    ...
    workload = Workload(
        options.out_dir,
        in_compilation=True,
        use_real_data=options.true_data,
