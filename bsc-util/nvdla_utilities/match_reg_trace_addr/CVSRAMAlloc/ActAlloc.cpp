...
struct TensorAllocInfo
{
    ...
    TensorAllocInfo(uint64_t start, uint64_t end,
        uint64_t size, uint32_t accesses) :
            use_start(start), use_end(end),
            size(size), num_access(accesses) {}
};
...
public:
    ...
    NoChangeTimeoutCallback(time_t _time = 60, time_t abs_timeout = 600,
            time_t base_time = 300, double _epsilon = 0.01):
        timeout_time(_time), abs_timeout_time(abs_timeout),
        base_sol_time(base_time), epsilon(_epsilon), last_gap(-1),
            ...
            if (time(nullptr) - init_time > base_sol_time) {
                if (fabs(this_gap - last_gap) < epsilon * this_gap) {
                    if (time(nullptr) - last_time > timeout_time) {
                        std::cout << "gap has not changed for more than "
                                  << timeout_time << " seconds. Abort.\n";
                        abort();
                    }
                } else if (time(nullptr) - init_time > abs_timeout_time) {
                    std::cout << "has consumed over " << abs_timeout_time
                              << " seconds, abort\n";
                    abort();
                } else {
                    last_gap = this_gap;
...
public:
    AllocProblem(int argc, char** argv) {
        ...
        while (std::getline(fin, line)) {
            ...
            max_life_span = max_life_span > (end - start)
                ? max_life_span : (end - start);
        ...
        for (uint32_t i = 0; i < tensors.size(); i++)
            for (uint32_t j = i + 1; j < tensors.size(); j++)
                if (min(tensors[i].use_end, tensors[j].use_end)
                        > max(tensors[i].use_start, tensors[j].use_start))
        ...
        for (auto& tensor: tensors) {
            ...
            while (true) {
                if ((tensor.size >> this_log_divider) << this_log_divider
                        == tensor.size) {
                    this_log_divider++;
                } else {
                    break;
            ...
            this_log_divider--;
            log_divider = log_divider < this_log_divider
                ? log_divider : this_log_divider;
        }
        align = (1 << log_divider);

        for (auto& tensor: tensors) {
            tensor.size = (tensor.size - 1) / align + 1;
            max_tensor_size = tensor.size > max_tensor_size
                ? tensor.size : max_tensor_size;
        }
    ...
    void FormulateILP() {
        try {
            ...
            for (uint32_t i = 0; i < tensors.size(); i++) {
                pos.emplace_back(model.addVar(0, mem_size - 1, 0, GRB_INTEGER,
                    std::string("pos") + std::to_string(i)));
                sel.emplace_back(model.addVar(0, 1, 0, GRB_BINARY,
                    std::string("sel") + std::to_string(i)));
            }

            for (uint32_t i = 0; i < overlap.size(); i++) {
                sij.emplace_back(model.addVar(0, 1, 0, GRB_BINARY,
                    std::string("sij") + std::to_string(i)));
                bij.emplace_back(model.addVar(0, 1, 0, GRB_BINARY,
                    std::string("bij") + std::to_string(i)));
            }

            GRBLinExpr obj = 0;
            for (uint32_t i = 0; i < tensors.size(); i++) {
                obj += sel[i] * tensors[i].size
                        * tensors[i].num_access / max_tensor_size;
            ...
            // cannot exceed bound
            for (uint32_t i = 0; i < tensors.size(); i++)
                model.addConstr(pos[i] + tensors[i].size - max_tensor_size
                        * (1 - sel[i]) <= mem_size);

            // either i is beneath j
            for (uint32_t i = 0; i < overlap.size(); i++) {
                uint32_t first = overlap[i].first;
                model.addConstr(pos[first] + tensors[first].size - mem_size
                        * bij[i] - max_tensor_size * (1 - sij[i])
                        <= pos[overlap[i].second]);
            }

            // or j is beneath i
            for (uint32_t i = 0; i < overlap.size(); i++) {
                uint32_t second = overlap[i].second;
                model.addConstr(pos[second] + tensors[second].size - mem_size
                    * (1 - bij[i]) - max_tensor_size
                    * (1 - sij[i]) <= pos[overlap[i].first]);
            }

            // sij = sel[i] * sel[j]
            for (uint32_t i = 0; i < overlap.size(); i++) {
                model.addConstr(sij[i] >= sel[overlap[i].first]
                    + sel[overlap[i].second] - 1);
                model.addConstr(sij[i] <= sel[overlap[i].first]);
            ...
            for (uint32_t i = 0; i < sel.size(); i++) {
                fout << int(sel[i].get(GRB_DoubleAttr_X)) << " "
                     << int(pos[i].get(GRB_DoubleAttr_X)) * align << "\n";
            }
