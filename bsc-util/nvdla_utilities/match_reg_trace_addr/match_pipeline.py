...
class PipelineMultiBatch:
    ...
    def remap_weights(self):
        ...
        all_weights.sort(
            key=lambda x: self.pipeline_stages[x[0]].data[(x[1], x[2])].size,
            reverse=True,
        )
        ...
        for weight_desc in all_weights:
            ...
            next_avail_aligned = self.aligned_ceil(
                next_avail_aligned
                + self.pipeline_stages[weight_desc[0]]
                .data[(weight_desc[1], weight_desc[2])]
                .size
            )
    ...
    def remap_activations(self):
        ...
        all_activations.sort(
            key=lambda x: self.pipeline_stages[x[1]].data[(x[2], x[3])].size,
            reverse=True,
        )
        ...
        for act_desc in all_activations:
            ...
            next_avail_aligned = self.aligned_ceil(
                next_avail_aligned
                + self.pipeline_stages[act_desc[1]]
                .data[(act_desc[2], act_desc[3])]
                .size
            )
        ...
        for stage_id in range(1, len(self.pipeline_stages)):
            for i, ipt in enumerate(self.pipeline_stages[stage_id].inputs):
                for batch_id in range(self.batch_num):
                    ...
                    self.activation_map[key] = self.activation_map[
                        corr_opt_key
                    ]
...
def parse_args():
    ...
    parser.add_argument(
        "--out-dir",
        default="/home/lactose/nvdla/traces/lenet_auto_pipeline_multibatch/",
...
def main():
    ...
    for i in range(len(options.in_dir)):
        os.makedirs(
            os.path.join(options.out_dir, "stage_" + str(i + 1)), exist_ok=True
        )
