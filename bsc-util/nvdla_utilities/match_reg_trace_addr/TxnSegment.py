...
corr_map_table = {
    0x5000: [0x5000, 0x6000],
    0xB000: [0xA000, 0xB000],
    0xC000: [0xC000, 0xD000],
}
...
class TxnSegment:
    def __init__(self, core_reg, core_reg_val, core_line_num, file_name):
        ...
        self.match_candidates = (
            []
        )  # the indices to its possible matched txn segments in the unpartitioned txn file
        self.matched_target = (
            None  # the index to its matched target txn segment
        )
    ...
    def desc_str(self):
        return (
            self.file_name
            + " line "
            + str(self.core_line_num)
            + " ("
            + str(hex(self.core_reg))
            + " = "
...
def expand_line_range(txn_words, pivot_line_idx, txn_seg):
    ...
    while True:
        ...
        if (
            txn_words[lower_line_idx][0] != "write_reg"
            or reg_addr & 0xFF000 not in corr_map_table[pivot_reg_base]
            or ((reg_addr & 0xFFF == 0x004) or (reg_addr & 0xFFF == 0x000))
            or reg_addr in reg_to_neglect
        ):
            patience -= 1
            if patience <= 0:
                break
        ...
        if reg_addr in addr_reg_list:
            ...
            if reg_addr not in txn_seg.addr_reg_mapping:
                txn_seg.addr_reg_mapping[reg_addr] = int(
                    txn_words[lower_line_idx][2], 16
                )
        else:
            if reg_addr not in txn_seg.other_reg_mapping:
                txn_seg.other_reg_mapping[reg_addr] = int(
                    txn_words[lower_line_idx][2], 16
                )
    ...
    while True:
        ...
        if (
            txn_words[upper_line_idx][0] != "write_reg"
            or reg_addr & 0xFF000 not in corr_map_table[pivot_reg_base]
            or ((reg_addr & 0xFFF == 0x004) or (reg_addr & 0xFFF == 0x000))
            or reg_addr in reg_to_neglect
        ):
...
def get_txn_segments(txn_words, file_name):
    ...
    for i in range(line_num):
        ...
        if txn_words[i][3] in pivot_addr_reg_name_list:
            txn_segment = TxnSegment(
                int(txn_words[i][3], 16),
                int(txn_words[i][2], 16),
                i,
                file_name,
            )
