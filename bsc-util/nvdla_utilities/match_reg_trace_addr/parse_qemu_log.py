...
class Data:
    ...
    def valid(self):
        if self.addr_id <= 0 or (
            self.addr_id == 0 and self.offset == 0 and self.size == 0
        ):
            return False
        return True
...
class TensorSurface:
    def __init__(self, tsd, tb, addr_id, offset, addr_base_map):
        ...
        self.tb_name = (
            tb  # but multiple Data objects may belong to the same tb
        )
...
class TensorBuffer:
    ...
    def add_tensor_surface(self, ts_name, ts, addr_base_map):
        ...
        if (
            ts.addr_id in addr_base_map
        ):  # otherwise this TensorBuffer will be deleted. Don't care
            if self.offset is None:
                self.addr_id = (
                    ts.addr_id
                )  # self.addr_id can be None during construction
...
class Workload:
    def __init__(
        ...
        self.addr_base_map = (
            {}
        )  # {addr_base_id: addr_base_val}; addr_base_val starts from 0xc0000000
        ...
        self.txn_lines = (
            []
        )  # for self.sclog2traces() to store translated contents of input.txn
        self.in_compilation = in_compilation  # constructor called in compilation or remapping phase
        self.use_real_data = (
            use_real_data  # (does not contain dump_mem instructions)
        )
        self.dump_results = (
            dump_results  # whether to dump results for checking correctness
        )
        ...
        if (
            self.in_compilation
        ):  # memory trace files and input.txn not yet generated
        ...
        mem_trace_path = os.path.join(
            self.in_dir,
            "VP_mem_rd_wr" if self.in_compilation else "rtl_mem_rd_wr",
        )
        if not self.in_compilation:
            # check validity of rtl_mem_rd_wr and nvdla_cpp.log
            nvdla_cpp_log = os.path.abspath(
                os.path.join(self.in_dir, "nvdla_cpp.log")
            )
            legal = (
                os.path.exists(nvdla_cpp_log)
                and os.path.exists(mem_trace_path)
                and os.stat(nvdla_cpp_log).st_size != 0
                and os.stat(mem_trace_path).st_size != 0
            )
            log_tail_lines = (
                "".join(os.popen("tail -n 3 " + nvdla_cpp_log).readlines())
                if legal
                else ""
            )
            legal = (
                legal
                and ("done at" in log_tail_lines)
                and ("PASS" in log_tail_lines or "FAIL" in log_tail_lines)
            )
            if not legal:
                usr_pfx = (
                    os.popen("cd ~/ && pwd").readlines()[0].strip().rstrip("/")
                )
                bin_dir_in_docker = os.path.abspath(
                    os.path.join(
                ...
                for line in os.popen("docker images").readlines():
                    words = line.split()
                    if (
                        "edwinlai99/gem5_nvdla_env" in words[0]
                        and "v3" in words[1]
                    ):
                ...
                cid = (
                    os.popen(
                        "docker run -it -d --rm -v ~/:/home edwinlai99/gem5_nvdla_env:v3"
                    )
                    .readlines()[0]
                    .strip()
                )
                ...
                for line in nvdla_cpp_log_lines:
                    rw_m = re.search(
                        "([a-z]+) request from dla, addr ([0-9a-zA-Z]+)", line
                    )
                    if rw_m is not None:
                ...
                with open(
                    os.path.join(self.in_dir, "rtl_mem_rd_wr"), "w"
                ) as fp:
        ...
        self.addr_log, self.sorted_addr, self.raw_addr_log = parse_rd_wr_trace(
            mem_trace_path
        )
        ...
        for rw_and_addr in self.raw_addr_log:
            ...
            if (
                "r" in rw_and_addr[0]
                and rw_and_addr[1] in rd_only_addr2tb_name.keys()
            ):
        ...
        if self.in_compilation and self.use_real_data:
            ...
            for line in sclog_lines:
                if "Info: nvdla.dbb_adaptor: GP: iswrite" in line:
                    info_match = re.search(
                        "addr=([0-9a-zA-Z]+) len=([0-9]+) data=0x ([0-9a-fA-FX_ ]+) resp",
                        line,
                    )
            ...
            to_get_tb = (
                self.w_tb + self.in_tb + self.out_tb
                if self.dump_results
                else self.w_tb + self.in_tb
            )
            for tb_name in to_get_tb:
                ...
                aligned_end_ceil = (
                    (tb.addr + tb.size - 1) // self.axi_width + 1
                ) * self.axi_width
                # true_start = data_blk.addr
                # true_end = data_blk.addr + data_blk.size
                for addr in range(
                    aligned_start, aligned_end_ceil, self.axi_width
                ):
            ...
            self.txn_lines = (
                to_prepend_txn_lines + self.txn_lines + to_append_txn_lines
            )
        ...
        for line_id, line in enumerate(lines):
            if "(Surface) Address list entry for" in line:
                name_match = re.search(
                    r"tsd=(tsd-\d+)/(tb-\d+):\d+ -> (\d+) offset=(\d+) size=(\d+)",
                    line,
                )
                ...
                tb_size = int(
                    name_match.group(5)
                )  # this tb_size is not 100% correct, as observed in resnet50 inputs
    ...
    def sclog2traces(self):
        ...
        for sclog_line in sclog_lines:
            if len(sclog_line) < 2:  # the line is empty
                continue
            if (
                "NV_NVDLA_csb_master::nvdla2csb_b_transport, base_addr:"
                in sclog_line
            ):
                continue
            if (
                "NV_NVDLA_csb_master::nvdla2csb_b_transport, csb req to"
                in sclog_line
            ):
                ...
                info = re.search(
                    "NV_NVDLA_csb_master.cpp: [0-9]+:([A-Za-z ]+): ([0-9a-zA-Z]+)",
                    sclog_line,
                )
                if info is not None:
                    key = info.group(1)
                    ...
                    if key == "Addr":
                        ...
                        if csb_is_write != 0:  # the end for csb write_reg log
                            csb_inputting = False
                            out_addr = 0xFFFF0000 + (
                                0x0000FFFF & ((csb_reg - 0) >> 2)
                            )
                            # write bit will be corrected by txn2verilator
                            txn_lines.append(
                                "write_reg 0x%x 0x%08x\t\t\t#0x%04x\n"
                                % (out_addr, csb_data, csb_reg)
                            )
                else:
                    exp = re.search(
                        "Err bit: ([0-9a-zA-Z]+) Data: ([0-9a-zA-Z]+)",
                        sclog_line,
                    )
                    # unique for csb read_reg response
                    assert exp is not None, (
                        "Unresolved csb line: " + sclog_line
                    )
                    ...
                    if csb_reg == 0x000C and csb_exp_data != 0:
                        ...
                        txn_lines.append(
                            "read_reg 0x%08x 0xffffffff 0x%08x\t#0x%04x\n"
                            % (out_addr, csb_exp_data, csb_reg)
                        )
...
def construct_surfaces(lines):
    ...
    for i, line in enumerate(lines):
        op_start_mark = re.findall(
            r"NVDLA FW ROI\[\d+\]: dla_[a-zA-Z]+_surface_desc", line
        )
        data_start_mark = re.findall(
            r"[a-zA-Z0-9]+_data\s+=\s+\[\s+dla_data_cube =>", line
        )
        ...
        if logging_data:
            ...
            if (
                " ]" in line
            ):  # the space before "]" differentiates from the [] for reporting time
                logging_data = False
                if temp_data.valid():
...
def get_addr_mapping(lines):
    addr_base_map = {}
    for line in lines:
        addr_base_match = re.search(
            r", got dst_ptr = ([0-9a-f]+) \(index = (\d+)\)", line
        )
        if addr_base_match is not None:
            ...
            if key not in addr_base_map.keys():
                ...
                assert (
                    addr_base_map[key] == val
                )  # the log file should agree with itself
    ...
    for line_id, line in enumerate(txn_lines):
        if "until" in line:
            ...
            for explore_id in range(line_id - 1, -1, -1):
                ...
                reg_match = re.search(
                    r"#(0x[0-9a-f]{1,2}0[0-9a-f]{2})", txn_lines[explore_id]
                )
                if (
                    reg_match is None
                    or int(reg_match.group(1), 16) in interrupt_regs
                ):
                    explore_id -= 1
                    continue
                else:
                    ...
                    if (
                        reg_addr in status_regs
                        or reg_addr in pingpong_regs
                        or reg_addr in kickoff_regs
                    ):
                        of_concern = False
                    else:
            ...
            if to_move_id_start is not None:
                ...
                for i in range(to_move_id_end - to_move_id_start):
                    new_lines.insert(
                        to_insert_pos + i, txn_lines[to_move_id_start + i]
                    )
