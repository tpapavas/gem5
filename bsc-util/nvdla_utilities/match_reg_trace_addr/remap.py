...
class BaseRemapper:
    ...
    def testcase_init(self, out_dir, sim_dir, testcase_str):
        ...
        self.sim_dir_host = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../../../mnt" + sim_dir)
        )
    ...
    def copy_output_to_img(self):
        os.system("mkdir -p " + self.sim_dir_host)
...
class IdentityRemapper(BaseRemapper):
    ...
    def testcase_init(self, out_dir, sim_dir, testcase_str=""):
        ...
        print(
            "[remap.py/IdentityRemapper] BaseRemapper.testcase_init(self, out_dir, sim_dir, testcase_str)"
        )
    ...
    def write_to_files(self):
        ...
        os.system(
            ...
            + script_path
            + " "
            + os.path.join(self.out_dir, "input.txn")
            + " "
        ...
        if not os.path.exists(rd_var_log_path):
            with open(rd_var_log_path, "wb") as fp:
                for rd_only_var in self.workload.rd_only_tbs:
                    ...
                    fp.write(
                        tb.addr.to_bytes(4, byteorder="little", signed=False)
                    )
                    fp.write(
                        tb.size.to_bytes(4, byteorder="little", signed=False)
                    )
...
class CVSRAMRemapper(BaseRemapper):
    ...
    def testcase_init(self, out_dir, sim_dir, testcase_str):
        BaseRemapper.testcase_init(self, out_dir, sim_dir, testcase_str)
        print(
            "[remap.py/CSVRAMRemapper] BaseRemapper.testcase_init(self, out_dir, sim_dir, testcase_str)"
        )
    ...
    def set_cvsram_param(self, num_cvsram, cvsram_base_addrs, cvsram_sizes):
        ...
        print(
            "[remap.py/CSVRAMRemapper] self.num_cvsram: ",
            self.num_cvsram,
            " self.cvsram_base_addrs: ",
            self.cvsram_base_addrs,
            " self.cvsram_sizes: ",
    ...
    def change_ram_type_to_cvsram(
        self, raw_lines, modified_lines, line_id, modify_status
    ):
        ...
        while explore_id >= 0:
            ram_reg_match = re.search(
                r"#\s?(0x[0-9a-f]{1,2}0[0-9a-f]{2})", raw_lines[explore_id]
            )
            if ram_reg_match is None:
                break
            if (
                ram_reg_match.group(0)[2] != reg_match.group(0)[2]
                or ram_reg_match.group(0)[3] != reg_match.group(0)[3]
            ):
                # only explore registers in the same reg group and continuous lines
                break
            if (
                int(ram_reg_match.group(1), 16) == ram_type_reg
                and not modify_status[explore_id]
            ):
                reg_val_match = re.search(
                    r"_reg\s0xffff[0-9a-f]{4}\s(0x[0-9a-f]{8})",
                    raw_lines[explore_id],
                )
                old_val_str = reg_val_match.group(1)
                new_val = int(old_val_str, 16) & ~(
                    1 << ram_type_bit
                )  # change to CVSRAM
                new_val_str = f"{new_val:#0{10}x}"
                modified_lines[explore_id] = new_val_str.join(
                    raw_lines[explore_id].rsplit(old_val_str, 1)
                )  # reverse_replace
...
class SingleAccelCVSRAMRemapper(CVSRAMRemapper):
    ...
    def testcase_init(self, out_dir, sim_dir, testcase_str):
        BaseRemapper.testcase_init(self, out_dir, sim_dir, testcase_str)
        print(
            "[remap.py/SingleAccelCVSRAMRemapper] BaseRemapper.testcase_init(self, out_dir, sim_dir, testcase_str)"
        )
        self.mapping.clear()
    ...
    def set_cvsram_param(self, num_cvsram, cvsram_base_addrs, cvsram_sizes):
        CVSRAMRemapper.set_cvsram_param(
            self, num_cvsram, cvsram_base_addrs, cvsram_sizes
        )
        print(
            "[remap.py/SingleAccelCSVRAMRemapper] self.num_cvsram: ",
            self.num_cvsram,
            " self.cvsram_base_addrs: ",
            self.cvsram_base_addrs,
            " self.cvsram_sizes: ",
    ...
    def write_to_files(self):
        ...
        modify_status = [
            False for _ in range(len(raw_txn_lines))
        ]  # False means not modified by remapping yet
        ...
        for orig_addr, mapped_addr in self.mapping.items():
            for line_id, line in enumerate(raw_txn_lines):
                if hex(orig_addr) in line and not modify_status[line_id]:
                    new_lines[line_id] = line.replace(
                        hex(orig_addr), hex(mapped_addr), 1
                    )
                    ...
                    self.change_ram_type_to_cvsram(
                        raw_txn_lines, new_lines, line_id, modify_status
                    )

        out_txn_path = os.path.join(
            self.out_dir, self.testcase_str + "_input.txn"
        )
        rd_var_log_path = os.path.join(
            self.out_dir, self.testcase_str + "_rd_only_var_log"
        )
...
class WeightPinRemapper(SingleAccelCVSRAMRemapper):
    def __init__(self, in_dir, model_name):
        ...
        self.weights_cp.sort(
            key=lambda x: self.workload.tb[x].size, reverse=True
        )
    ...
    def compute_remap_decision(self):
        ...
        for weight in self.weights_cp:
            ...
            if aligned_size <= space_left:
                self.mapping[self.workload.tb[weight].addr] = (
                    cvsram_base_addr + cvsram_size - space_left
                )
...
class ActPinRemapper(SingleAccelCVSRAMRemapper):
    ...
    def compute_remap_decision(self):
        ...
        gurobi_out_path = os.path.join(
            self.out_dir, self.testcase_str + "_alloc_result"
        )
        os.system(
            "cd "
            + os.path.join(os.path.dirname(__file__), "CVSRAMAlloc")
            + " && make ActAlloc"
        )
        return [
            "cd "
    ...
    def collect_remap_decision(self):
        ...
        relative_map = collect_gurobi_results(
            self.workload,
            self.cvsram_sizes[0],
            gurobi_out_path,
            gurobi_in_path,
...
class MixPinRemapper(SingleAccelCVSRAMRemapper):
    ...
    def compute_remap_decision(self):
        ...
        gurobi_out_path = os.path.join(
            self.out_dir,
            self.testcase_str.replace("MixPin", "ActPin") + "_alloc_result",
        )
        os.system(
            "cd "
            + os.path.join(os.path.dirname(__file__), "CVSRAMAlloc")
        ...
        return [
            ...
            + os.path.join(
                self.out_dir,
                self.testcase_str.replace("MixPin", "ActPin")
                + "_gurobi_stdout",
            )
        ]
    ...
    def collect_remap_decision(self):
        ...
        for tb_name, rel_addr in relative_map.items():
            ...
            cvsram_space_left = min(
                cvsram_space_left,
                self.cvsram_sizes[0] - rel_addr - buffer.size,
            )
...
class PipelineRemapper(BaseRemapper):
    def __init__(self, in_dir, model_name):
        ...
        self.all_w_tb.sort(
            key=lambda x: self.pipeline_stages[x[0]].tb[x[1]].size,
            reverse=True,
        )
        ...
        self.inter_act_map = (
            {}
        )  # {(batch_id, stage_id, tb_name): (mapped_addr, is_cvsram)}
        self.intra_act_map = (
            {}
        )  # {(stage_id, tb_name): (mapped_addr, is_cvsram)}

        self.weight_addr_map = (
            {}
        )  # {(stage_id, orig_addr): (mapped_addr, is_cvsram)}
        self.inter_act_addr_map = (
            {}
        )  # {(batch_id, stage_id, orig_addr): (mapped_addr, is_cvsram)}
        self.intra_act_addr_map = (
            {}
        )  # {(stage_id, orig_addr): (mapped_addr, is_cvsram)}
    ...
    def testcase_init(self, out_dir, sim_dir, testcase_str):
        BaseRemapper.testcase_init(self, out_dir, sim_dir, testcase_str)
        print(
            "[remap.py/PipelineRemapper] BaseRemapper.testcase_init(self, out_dir, sim_dir, testcase_str)"
        )
    ...
    def set_pipeline_params(self, num_batches):
        self.batch_num = num_batches
        print(f"[remap.py/PipelineRemapper] batch_num = {self.batch_num}")

        # Βασικά checks/σύνοψη
        nstages = len(getattr(self, "pipeline_stages", []))
        print(f"[remap.py/PipelineRemapper] #stages = {nstages}")
        if nstages == 0:
            print("[remap.py/PipelineRemapper] WARNING: pipeline_stages is empty!")
            return

        # Προαιρετικό: μικρό helper για ασφαλή πρόσβαση
        def _list_attr(obj, name):
            val = getattr(obj, name, None)
            if val is None:
                print(f"[remap.py/PipelineRemapper]   WARN: stage has no attr '{name}'")
                return []
            return val

        # Καθάρισε buffers αν χρειάζεται
        if not hasattr(self, "intra_act_tb"): self.intra_act_tb = []
        if not hasattr(self, "inter_act_tb"): self.inter_act_tb = []
        self.intra_act_tb.clear()
        self.inter_act_tb.clear()

        # Αναλυτικά prints ανά στάδιο
        for i, stage in enumerate(self.pipeline_stages):
            print(f"[remap.py/PipelineRemapper] --- Stage {i} ---")
            in_tb      = _list_attr(stage, "in_tb")
            itm_act_tb = _list_attr(stage, "itm_act_tb")
            out_tb     = _list_attr(stage, "out_tb")

            print(f"[remap.py/PipelineRemapper]   in_tb len={len(in_tb)}: {in_tb}")
            print(f"[remap.py/PipelineRemapper]   itm_act_tb len={len(itm_act_tb)}: {itm_act_tb}")
            print(f"[remap.py/PipelineRemapper]   out_tb len={len(out_tb)}: {out_tb}")

            # Εσωτερικές ενεργοποιήσεις (δεν εξαρτώνται από batch)
            for act in itm_act_tb:
                print(f"[remap.py/PipelineRemapper]   append intra_act_tb: (stage={i}, act={act})")
                self.intra_act_tb.append((i, act))

            # Εξερχόμενες ενεργοποιήσεις (ανά batch)
            for act in out_tb:
                for batch in range(self.batch_num):
                    print(f"[remap.py/PipelineRemapper]   append inter_act_tb: (batch={batch}, stage={i}, act={act})")
                    self.inter_act_tb.append((batch, i, act))

        # Είσοδοι μόνο από το πρώτο στάδιο (ανά batch)
        first_in = _list_attr(self.pipeline_stages[0], "in_tb")
        print(f"[remap.py/PipelineRemapper] Stage 0 in_tb for inter-batch mapping: len={len(first_in)}")
        for act in first_in:
            for batch in range(self.batch_num):
                print(f"[remap.py/PipelineRemapper]   append inter_act_tb (input): (batch={batch}, stage=0, act={act})")
                self.inter_act_tb.append((batch, 0, act))

        # Τελικές σύνοψεις
        print(f"[remap.py/PipelineRemapper] intra_act_tb total = {len(self.intra_act_tb)}")
        print(f"[remap.py/PipelineRemapper] inter_act_tb total = {len(self.inter_act_tb)}")
    """
    ...
    def remap_weights(self):
        ...
        for w_desc in self.all_w_tb:
            ...
            self.weight_map[w_desc] = (
                next_avail_aligned,
                False,
            )  # False means it isn't mapped to CVSRAM
            self.weight_addr_map[(w_desc[0], w_tb.addr)] = (
                next_avail_aligned,
                False,
            )
            # aligned size
    ...
    def remap_activations(self):
        ...
        for act_id, act_desc in enumerate(
            self.inter_act_tb + self.intra_act_tb
        ):
            ...
            activation_map[act_desc] = (
                next_avail_aligned,
                False,
            )  # False means it isn't mapped to CVSRAM
            aligned_size = self.aligned_ceil(
                self.pipeline_stages[act_desc[-2]].tb[act_desc[-1]].size
            )
        ...
        for stage_id in range(1, len(self.pipeline_stages)):
            for i, ipt in enumerate(self.pipeline_stages[stage_id].in_tb):
                for batch_id in range(self.batch_num):
                    ...
                    corr_opt = self.pipeline_stages[stage_id - 1].out_tb[
                        i
        ...
        for key, val in activation_map.items():
            ...
            if len(key) == 2:  # intermediate activation
                ...
                for tsd_name in tb.tsd_list:
                    ...
                    self.intra_act_addr_map[(key[-2], tsd_addr)] = (
                        val[0] + tsd_addr - tb.addr,
                        val[1],
                    )
            else:
                assert len(key) == 3
                ...
                for tsd_name in tb.tsd_list:
                    tsd_addr = stage.ts[tsd_name].addr
                    self.inter_act_addr_map[(key[-3], key[-2], tsd_addr)] = (
                        val[0] + tsd_addr - tb.addr,
                        val[1],
                    )
        ...
        for i in range(self.num_stages):
            ...
            for batch_id in range(self.batch_num):
                ...
                for w_addr_desc, w_map_info in self.weight_addr_map.items():
                    ...
                    if w_addr_desc[0] == i:
                        ...
                        for line_id, line in enumerate(txn_lines):
                            if (
                                hex(orig_addr) in line
                                and not modify_status[line_id]
                            ):
                                new_lines[line_id] = line.replace(
                                    hex(orig_addr), hex(addr_mapped)
                                )
                                modify_status[line_id] = True
                                if is_cvsram:
                ...
                for (
                    ia_addr_desc,
                    ia_map_info,
                ) in self.intra_act_addr_map.items():
                ...
                for (
                    io_addr_desc,
                    io_map_info,
                ) in self.inter_act_addr_map.items():
                ...
                out_txn_path = os.path.join(
                    os.path.abspath(self.out_dir), out_pfx + "input.txn"
                )
                out_bin_path = os.path.join(
                    os.path.abspath(self.out_dir), out_pfx + "trace.bin"
                )
                rd_var_log_path = os.path.join(
                    os.path.abspath(self.out_dir), out_pfx + "rd_only_var_log"
                )
...
class PipelineWeightPinRemapper(CVSRAMRemapper, PipelineRemapper):
    ...
    def remap_weights(self):
        next_avail_aligned = (
            self.weight_base_addr
        )  # next available addr in DRAM
        spaces_left = [
            self.cvsram_sizes[i] for i in range(self.num_cvsram)
        ]  # available space in all CVSRAMs
        for weight_desc in self.all_w_tb:
            ...
            if (
                aligned_size <= spaces_left[stage_id]
            ):  # like WeightPinRemapper, assign weights greedily
                map_to_addr = (
                    self.cvsram_base_addrs[stage_id]
                    + self.cvsram_sizes[stage_id]
                    - spaces_left[stage_id]
                )
                self.weight_map[weight_desc] = (map_to_addr, True)
                self.weight_addr_map[(stage_id, w_tb.addr)] = (
                    map_to_addr,
                    True,
                )
...
class PipelineActPinRemapper(CVSRAMRemapper, PipelineRemapper):
    ...
    def remap_activations(self):
        ...
        for stage_id in range(len(self.in_dirs)):
            itm_acts_file = os.path.join(
                self.in_dirs[stage_id], "intermediate_acts"
            )
            write_solver_input(
                itm_acts_file,
                self.pipeline_stages[stage_id],
                log_weights=False,
            )

            # call the gurobi solver
            stage_out_dir = os.path.join(
                self.out_dir, "stage_" + str(stage_id + 1)
            )
            os.makedirs(stage_out_dir, exist_ok=True)
            gurobi_out_path = os.path.join(
                stage_out_dir, self.testcase_str + "_alloc_result"
            )
            os.system(
                "cd "
                + os.path.join(os.path.dirname(__file__), "CVSRAMAlloc")
            ...
            cmds.append(
                ...
                + gurobi_out_path
                + " "
                + str(self.cvsram_sizes[stage_id])
                + " > "
                + os.path.join(
                    stage_out_dir, self.testcase_str + "_gurobi_stdout"
                )
            )
        return cmds
    ...
    def collect_remap_decision(self):
        ...
        for stage_id in range(self.num_stages):
            ...
            for tb_name, rel_addr in relative_map.items():
                ...
                self.intra_act_map[global_key] = (
                    self.cvsram_base_addrs[stage_id] + rel_addr,
                    True,
                )
                tb = stage.tb[tb_name]
                for tsd_name in tb.tsd_list:
...
def write_solver_input(file_path, workload, log_weights):
    with open(file_path, "w") as fp:
        for act in workload.itm_act_tb:
            ...
            fp.write(
                str(buffer.liveness[0])
                + " "
                + str(buffer.liveness[1])
                + " "
                + str(buffer.size)
                + " "
                + str(buffer.num_access)
                + " "
                + hex(buffer.addr)
                + " "
            )
            last_aligned_addr = last_aligned(
                buffer.addr, buffer.size, workload.axi_width
            )
...
def collect_gurobi_results(
    workload, cvsram_size, gurobi_out_path, gurobi_in_path
):
    # read results from the solver and draw the CVSRAM occupation figure
    ...
    relative_mapping = (
        {}
    )  # {data_desc(inside a pipeline stage): mapping position in a CVSRAM}
    for line_id, line in enumerate(out_lines):
        ...
        if words[0] == "1":
            tb_name = (
                workload.tb[workload.w_tb[collect_w_cnt]].tb_name
                if is_w
                else workload.itm_act_tb[collect_a_cnt]
            )
            ...
            if is_w:
                ...
                ax1.add_patch(
                    patches.Rectangle(
                        (buffer.liveness[0], int(words[1])),
                        buffer.liveness[1] - buffer.liveness[0],
                        buffer.size,
                        linewidth=1,
                        edgecolor="black",
                    )
    ...
    plt.title(
        "Buffer Allocation Result on CVSRAM size = 0x%x Bytes" % cvsram_size
    )
