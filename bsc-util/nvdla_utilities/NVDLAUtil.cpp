...
public:
    ...
    AXI_Txn(int _print_rd, int _print_wr, int addr_convert_flag)
        : print_rd_flag(_print_rd), print_wr_flag(_print_wr),
          change_addr_flag(addr_convert_flag) {}
...
public:
    ...
    CSB_Txn(int flag, int addr_convert_flag)
     : addr(0), data(0), is_write(0), n_posted(0), err_bit(0), exp_data(0),
       inputting(false), print_reg_txn_flag(flag),
       change_addr_flag(addr_convert_flag) {}
    ...
    void print_csb_txn() const {
        if (print_reg_txn_flag) {
            ...
            uint32_t out_addr =
                    0xffff0000 + (0x0000ffff & ((addr - 0) >> 2));
...
void parse_args(int argc, char** argv, int* flags,
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--input1") == 0 || strcmp(argv[i], "-i1") == 0
          || strcmp(argv[i], "--input2") == 0 || strcmp(argv[i], "-i2") == 0
          || strcmp(argv[i], "-i") == 0) {
          ...
          || strcmp(argv[i], "-h") == 0) {
            printf("Usage: ./NVDLAUtil [-i1] in_file_1 ([-i2] in_file_2) "
                "[--function <function>] [--print-options]\n\n\n");
            printf("\t--function | -f: select the function to operate on\n");
            printf("\t <function>: parse-vp-log (convert VP debug info "
                "to NVDLA register traces (.txn), memory traces or both)\n");
            printf("\t             comp-mem-trace "
                "(compare memory trace files)\n");
            printf("\t             nvdla-cpp-log2mem-trace "
                "(convert terminal output of nvdla.cpp to memory traces)\n\n");
            printf("\t[--print-options]:\n");
            printf("\t\t--print-mem-rd: print AXI data read addresses "
                "(aligned to 0x40)\n");
            printf("\t\t--print-mem-wr: print AXI data write addresses "
                "(aligned to 0x40)\n");
...
void VPLog2Txn(const std::string& vp_trace_file_name, const int* print_flags) {
    ...
    while (getline(vp_trace_file, vp_file_line)) {
        ...
        if (
            vp_file_line.find("NV_NVDLA_csb_master.cpp:") != std::string::npos
        ) {
            assert(csb_txn.inputting);
        ...
        if (vp_file_line.find("NvdlaAxiAdaptor::axi_rd_wr_thread, send")
          != std::string::npos) {
...
void CompMemTrace(const std::string& file_1_name,
  const std::string& file_2_name) {
    ...
    if (file_1_addr.size() != file_2_addr.size()) {
        printf("file size not equal, file_1_addr_num = %zu, "
               "file_2_addr_num = %zu\n",
...
void NVDLA_CPP_Log2MemTrace(const std::string& file_name,
        const int* print_flags) {
    ...
    while (getline(log_file, file_line)) {
        ...
        if (file_line.find("DBB: read request from dla, addr")
                != std::string::npos) {
        ...
        if (file_line.find("DBB: write request from dla, addr")
                != std::string::npos) {
            size_t pos_of_addr = file_line.find("addr") + 5;
            if (print_flags[1]) {
                for (size_t i = pos_of_addr; i < pos_of_addr + 8; i++)
