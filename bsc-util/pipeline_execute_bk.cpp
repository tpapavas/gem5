...
size_t get_trace_from_file(const std::string& file_name_prefix, char* dst) {
    ...
    // write the following 8 bytes (2 x uint32_t with 0xffffffff)
    // to mark the end of rd_only_var_log
    dst[size++] = 0xff;
    dst[size++] = 0xff;
    dst[size++] = 0xff;
    dst[size++] = 0xff;
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < worker_num; i++) {
        region_nvdlas[i] =
            aligned_alloc(sizeof(int)*16,REGION_NVDLA * batch_num);
    }

    uint32_t trace_sizes[worker_num * batch_num];
    for (int i = 0; i < worker_num * batch_num; i++) {
        std::string file_name_prefix = trace_file_prefix
            + std::to_string(i / worker_num + 1)
            + '_' + std::to_string(i % worker_num + 1);
        std::cout << " file_name_prefix: " << file_name_prefix << '\n';
        trace_sizes[i] =
            get_trace_from_file(file_name_prefix,
                (char*)(region_nvdlas[i % worker_num])
            + (i / worker_num) * REGION_NVDLA);
    }

    while (1) {
        for (int i = 0; i < worker_num; i++) {
            if (!accel_busy[i]
                && task_map[(i + 1) * map_time_size + wave_front[i]]
                    == NOT_LAUNCHED
                && task_map[(i + 1 - 1) * map_time_size + (wave_front[i] - 1)]
                    == FINISHED
                && task_map[(i + 1) * map_time_size + wave_front[i] - 1]
                    == FINISHED) {

                uint64_t trace_addr = (uint64_t)(region_nvdlas[i])
                    + (wave_front[i] - i - 1) * REGION_NVDLA;

                // accel id and batch id start from 1
                printf("NVDLA %d launched batch %d at t = %f\n", i + 1,
                    wave_front[i] - i, wall_time());

                m5_start_accel_id(
                    trace_addr,
                    trace_sizes[worker_num * (wave_front[i] - i - 1) + i],
                    trace_addr, i);
                accel_busy[i] = 1;
